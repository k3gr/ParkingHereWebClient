<template>
  <div
    class="offcanvas offcanvas-end spot-view"
    tabindex="-1"
    id="offcanvasRight"
    ...
    <div class="offcanvas-body">
      ...
      <div class="d-flex my-3 justify-content-start gap-4">
        ...
        <label
          ...
          :class="standardSpots.length > 0 ? 'btn-outline-primary' : 'btn-secondary disabled'"
          for="standard"
        ...
        <input
          ...
          id="bus"
          autocomplete="off"
          :disabled="busSpots.length == 0"
          v-model="getReservationParams.type"
          :value="SpotTypeEnum.Bus"
        />
        <label
          class="btn btn-outline-primary"
          :class="busSpots.length > 0 ? 'btn-outline-primary' : 'btn-secondary disabled'"
          for="bus"
          >{{ SpotTypeEnum.Bus }}</label
        >
        <input
          type="radio"
          class="btn-check"
          ...
          id="vip"
          autocomplete="off"
          :disabled="vipSpots.length == 0"
          v-model="getReservationParams.type"
          :value="SpotTypeEnum.Vip"
        />
        <label
          class="btn"
          :class="vipSpots.length > 0 ? 'btn-outline-dark ' : 'btn-secondary disabled'"
          for="vip"
      ...
      <div class="d-flex flex-column border rounded p-3">
        ...
        <div>
          <span>{{ $t('Period') }}: </span>
          <span class="fw-bold"
            >{{ moment(getReservationParams.startDate).format('DD.MM.yyyy') }} -
